...
public class MySnakeFrame extends JFrame {
	...
	private ArrayList<Cuadrado> manzanas = new ArrayList<>();;
	...
	// Constructor
	public MySnakeFrame() {
		snake = new Serpiente(this.getWidth(), this.getHeight());
		jugando = false;
		mostrarFinal = false;
		mostrado = false;
		pausado = false;
	...
	private void generarManzana() {
		...
		System.out.println(random);
		int y = random;
		Cuadrado manzana = new Cuadrado(x, y, 20, 25233, this.getWidth(), this.getHeight());
		manzana.setAsManzana();
		manzanas.add(manzana);
	}

	// han pulsado el botón de start, hay que ponerlo todo en orden.
	public void empezarDeNuevo() {
		snake = new Serpiente(this.getWidth(), this.getHeight()); // nueva y flamante serpiente
	...
	public boolean tocaManzana() {
		for (Cuadrado manzana : manzanas) {
			if (manzana.estaEncimaDe(snake.getCabeza())) {
	...
	public void pintarManzanas(Graphics2D g2d) {
		for (Cuadrado cuadrado : manzanas) {
			cuadrado.pintarse(g2d);
		}
	}
}
